...
def compute_revenue(req):
  ...
  for i in req.nodes:
    ...
    if 'GPU' in req.nodes[i]:  revenue += (req.nodes[i]['GPU']
    if 'Memory' in req.nodes[i]:  revenue += (req.nodes[i]['Memory']
...
def compute_cost(req):
  ...
  for i in req.nodes:
    ...
    if 'GPU' in req.nodes[i]:  cost += (req.nodes[i]['GPU']
    if 'Memory' in req.nodes[i]:  cost += (req.nodes[i]['Memory']
...
def compute_utils(physical):
  ...
  if 'GPU' in physical.nodes[0]:
    ...
    for i in physical.nodes:
      gpu_util += 1 - (physical.nodes[i]['GPU']/physical.nodes[i]['MaxGPU'])
      memory_util += 1 -
